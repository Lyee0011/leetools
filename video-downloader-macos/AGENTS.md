...
- Run node tests/source-smoke.js, node tests/runtime.js, node tests/macos-platform.js, node tests/yangshipin.js, node tests/browser-resolver.js, node tests/http-security.js on the development host. Use /bin/bash tests/smoke-macos.sh on a real Mac, with --browser for the optional local browser test.

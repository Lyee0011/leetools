...
**这是独立的 Mac 预览版，已在一台 Apple Silicon Mac（macOS 26.2）完成自动检查，原生界面和真实平台下载仍待手动确认。** Intel 与其他系统版本尚未实机验收；请先试用，再向其他人推荐。Windows 用户继续使用原来的 [video-downloader](../video-downloader)，其目录、发布包和下载地址不变。
...
2026-09-24 的本机安装修复了系统 Bash 3.2 对中文标点旁变量的解析及退出清理掩盖失败的问题；完整自动测试、匿名浏览器测试和实际空闲停止后重启已通过，详见测试记录。

...
(async () => {
  ...
  try {
    ...
    const config = JSON.parse((await request(port, { path: '/api/config' })).body);
    assert.strictEqual(config.app, 'shinewood-video-downloader-macos');
    assert.strictEqual((await request(port, { method: 'POST', path: '/api/shutdown', headers: { 'Content-Type': 'application/json' }, body: '{}' })).status, 403);
    const auth = { 'Content-Type': 'application/json', 'X-Video-Downloader-Token': token };
    assert.strictEqual((await request(port, { method: 'POST', path: '/api/shutdown', headers: auth, body: JSON.stringify({instance:'wrong',build:config.build}) })).status, 409);
    assert.strictEqual((await request(port, { method: 'POST', path: '/api/shutdown', headers: auth, body: JSON.stringify({instance:config.instance,build:config.build}) })).status, 200);
    await new Promise((resolve, reject) => {
      if (child.exitCode !== null) return resolve();
      const timer = setTimeout(() => reject(new Error('Authenticated stop did not exit')), 5000);
      child.once('exit', () => { clearTimeout(timer); resolve(); });
    });
    console.log('HTTP security tests passed, including authenticated shutdown and installation identity.');
  } finally {
